...
package com.microsoft.azure.sdk.iot.common.jproxy.impl;
...
public class ProxyNegotiationHandler
{
    ...
    static class ReadWriteState
    {
        ...
        {
            final StringBuilder builder = new StringBuilder().append("ReadWriteState[").append(isClientWriter ? "client" : "service").append("Writer, ").append("client=").append(clientAddress).append(", ").append("svc=").append(serviceAddress).append(", ").append(isRead ? "read" : "write").append("]");
    ...
    class ReadWriteHandler implements CompletionHandler<Integer, ReadWriteState>
    {
                ...
                case PROXY_NOT_STARTED:
                    if (readWriteState.isRead)
                    {
                        ...
                        if (bytesToRead > 0)
                        {
                            ...
                            final InetSocketAddress serviceAddress = new InetSocketAddress(hostNamePortParts[0], Integer.parseInt(hostNamePortParts[1]));
                            readWriteState.serviceAddress = serviceAddress.toString();
                            serviceSocket.connect(serviceAddress, readWriteState, new ServiceConnectCompletionHandler());
                        }
                        else
                        {
                            // unreachable code
        ...
        public void failed(Throwable exc, ReadWriteState attachment)
        {
            ...
            {
                TRACE_LOGGER.warn("readWriteHandlerFailed error=" + exc.getClass() + ", " + proxyConnectionState + ", " + attachment.toString());
            }
        }
        ...
        private void copyBytesBetweenClientAndService(final ReadWriteState readWriteState)
        {
            if (!readWriteState.isRead)
            {
                ...
                final AsynchronousSocketChannel readChannel = readWriteState.isClientWriter ? serviceSocket : clientSocket;
            ...
            {
                final AsynchronousSocketChannel writeChannel = readWriteState.isClientWriter ? clientSocket : serviceSocket;
        ...
        private String[] extractHostNamePort(final byte[] connectRequest)
        {
            ...
            return firstLine.replace("CONNECT ", "").replace(" HTTP/1.1", "").split(":");
        }
        ...
        private class ServiceConnectCompletionHandler implements CompletionHandler<Void, ReadWriteState>
        {
            ...
            public void completed(Void result, ReadWriteState proxyInitRead)
            {
                ...
                if (TRACE_LOGGER.isWarnEnabled())
                {
                    TRACE_LOGGER.warn("serviceConnectSuccess, " + proxyConnectionState + ", " + proxyInitRead.toString());
                }
                ...
                clientSocket.write(clientWriterState.buffer, clientWriterState, ReadWriteHandler.this);
                ...
                clientSocket.read(serviceWriterState.buffer, serviceWriterState, ReadWriteHandler.this);
            }
            ...
            public void failed(Throwable exc, ReadWriteState attachment)
            {
                if (TRACE_LOGGER.isWarnEnabled())
                {
                    TRACE_LOGGER.warn("serviceConnectFailed, error=" + exc.getClass() + ", " + proxyConnectionState + ", " + attachment.toString());
                }
