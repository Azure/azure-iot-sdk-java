...
# This APK file is generated by the Android build task and then is copied into the android test run task. If the APK file
# is missing, then either the copy failed, or the APK was never successfully built.
if [ ! -f "iot-e2e-tests/android2/app/build/outputs/apk/debug/app-debug.apk" ]; then
    echo 'Missing expected apk file. Android build task likely failed silently.'
    exit -1
fi

if [ ! -f "iot-e2e-tests/android2/app/build/outputs/apk/androidTest/debug/app-debug-androidTest.apk" ]; then
    echo 'Missing expected apk file. Android build task likely failed silently.'
    exit -1
fi
...
adb -e install -r iot-e2e-tests/android2/app/build/outputs/apk/debug/app-debug.apk
adb -e install -r iot-e2e-tests/android2/app/build/outputs/apk/androidTest/debug/app-debug-androidTest.apk
