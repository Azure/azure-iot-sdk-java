    ...
    public void onStatusChanged(ConnectionStatusChangeContext connectionStatusChangeContext) {
        ...
        if (shouldDeviceReconnect(status, statusChangeReason, throwable)) {
            if (suppliedCallbackPair != null) {
                suppliedCallbackPair.getKey().onStatusChanged(new ConnectionStatusChangeContext(DISCONNECTED_RETRYING, previousStatus, NO_NETWORK, throwable, suppliedCallbackPair.getValue()));
            }

            handleRecoverableDisconnection();
        } else if (suppliedCallbackPair != null) {
            suppliedCallbackPair.getKey().onStatusChanged(new ConnectionStatusChangeContext(status, previousStatus, statusChangeReason, throwable, suppliedCallbackPair.getValue()));
        }
    }
